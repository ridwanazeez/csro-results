...
- **StandingsView.vue** — Championship standings aggregator showing qualifying/race history, plus driver/team/country standings calculated from all saved RACE results
...
- **DO NOT** refactor to use `Date` objects — manual calculation is required for the game-specific format
...
- Returns formatted time strings in an array indexed by position
...
- **Team Standings** — Points grouped by `Driver.Team` from the Cars array
- **Country Standings** — Points grouped by `Driver.Nation` with country code lookup
- Only RACE type results count for standings; QUALIFY results are displayed separately
...
- **Assetto Corsa JSON format** — Expects a specific structure:
...
2. **GitHub Pages base path** — All asset references must work with the `/csro-results/` prefix
...
5. **Best lap highlighting** — Green background applied via `:class` binding to the fastest-lap row
